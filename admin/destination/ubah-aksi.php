<?php
    ...
    for($i=0; $i<sizeof($add_nama_gambar); $i++){
        if($add_nama_gambar[$i] != ''){
            ...
            $queryInsert = "INSERT INTO destinasi_gambar (destinasi_id,gambar) VALUES ('$id','$imageSave')";
    ...
    for($i=0; $i<sizeof($nama_gambar); $i++){
        $query_mysql = mysqli_query($koneksi,"SELECT gambar FROM destinasi_gambar WHERE id = '$idgambar[$i]'");    
        if($nama_gambar[$i]!=''){
            ...
            if(unlink('../../'.$data['gambar'])){
                ...
                $queryUpdate = "UPDATE destinasi_gambar SET gambar = '$imageSave' WHERE id = '$idgambar[$i]'";
